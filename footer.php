<footer class="jumbotron footer-bs">
        <div class="container">
            <div class = "row">
            <div class="col-md-5 footer-brand">
            <a href="#"> <img id="logo" alt="Logo" src="Images/LargeLogo.png" width="60" height="60"> <span style ="max-width: 70%; font-family: 'Crimson Text', serif; font-size: 170%">The Food App</span> </a> 
            
                <p>The Food App is an organization committed to helping consumers
                    remain aware of what they're eating and the environmental impact
                     of every bite. Our application allows users to instantly check 
                     the environmental cost of food items. To read more about our
                      mission and how we're increasing sustainability, visit the rest 
                      of our website.
                </p>
                <p>The Food App, all rights reserved</p>
            </div>
            
            <ul class="social list-inline col-md-3">
                    
                <li class="list-inline-item"><a href="mailto:dhsgirlsmatha@gmail.com" class="social-link"><i class="fas fa-envelope fa-3x"></i></a></li>
                <!--<li class="list-inline-item"><a href="https://www.instagram.com/dhs.girlsmathassociation/" class="social-link"><i class="fab fa-instagram fa-3x"></i></a></li>-->
                <li class="list-inline-item"><a href="https://discord.com/invite/tzZBVA8y" class="social-link"><i class="fab fa-linkedin fa-3x"></i></a></li>
                <li class="list-inline-item"><a href="#" class="social-link"><i class="fas fa-share fa-3x"></i></a></li>
                <li class="list-inline-item"><a href="mailto:dhsgirlsmatha@gmail.com" class="social-link"><i class="fas fa-envelope fa-3x"></i></a></li>
                <!--<li class="list-inline-item"><a href="https://www.instagram.com/dhs.girlsmathassociation/" class="social-link"><i class="fab fa-instagram fa-3x"></i></a></li>-->
                <li class="list-inline-item"><a href="https://discord.com/invite/tzZBVA8y" class="social-link"><i class="fab fa-linkedin fa-3x"></i></a></li>
                <li class="list-inline-item"><a href="#" class="social-link"><i class="fas fa-share fa-3x"></i></a></li>
            </ul>
            <div class="col-md-3 footer-nav">
                <h4>Menu —</h4>
                    <ul class="pages">
                        <li><a href="#">Home</a></li>
                        <li><a href="#">Pathway</a></li>
                        <li><a href="#">Guide</a></li>
                        <li><a href="#">School</a></li>
                    </ul>
            </div>
                   
            </div>
            <div class = "text-center"><h5><a style = "font-family: 'Crimson Text', serif; text-decoration: none" href= "#">Website and application created by Grace Liu & Ria Jain © 2021 </a> </h5></div>
    </footer>